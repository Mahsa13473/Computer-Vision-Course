function [img1] = myEdgeFilter(img0, sigma)
    ...
    smooth_img = myImageFilter(img0, gaussian_filter);

    % Sobel filters
    vertical_sobel_filter = fspecial('sobel');
    horizontal_sobel_filter = transpose(vertical_sobel_filter);

    % finding image gradients in x and y directions
    imgx = myImageFilter(smooth_img, horizontal_sobel_filter);
    imgy = myImageFilter(smooth_img, vertical_sobel_filter);
